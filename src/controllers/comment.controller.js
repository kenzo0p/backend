...
const updateComment = asyncHandler(async (req, res) => {
  ...
  if (!updateComment) {
    throw new ApiError(500, "error while upadating the comment");
  }
  return res
    .status(200)
    .json(new ApiResponse(200, updateComment, "Comment updated successfullu"));
});
...
const deleteComment = asyncHandler(async (req, res) => {
  ...
  if(comment.owner !== user){
    throw new ApiError(404,"you dont have permission to delete this comment")
  }

  const deleteComment = await Comment.findByIdAndDelete(commentId)
  if(!deleteComment){
    throw new ApiError(500,"error occur during deleting the comment")
  }
